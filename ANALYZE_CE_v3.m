...
start_clusters = [1	2	9	13	19	20	22	24	30	31	38	42	53	61	65	66];
...
out_CE = setdiff(1:M,start_clusters);       % which clusters aren't part of the starting clusters?

CE = sort(start_clusters); 
CE_orig = CE;
test_mat = MAT(:,CE);
...
EICs_w_clusnum = [CE_orig' final_coeffs_orig Estimated_ECI_errors' pcent_error' 2000*Estimated_ECI_errors'];
...
fprintf('\t   Cluster#\tECI     \tECI_error\t  percent_error error_range(meV)\n')
disp(EICs_w_clusnum)
fprintf('The chi value is:\n')
disp(chi)
fprintf('The RMSR is:\n')
disp(RMSE)
...
fprintf('----------------------------------------------------------------\n')
fprintf('----------------------------------------------------------------\n')
clearvars -except part_en ads_energy orig_surf_normalized_counts ECI_val atat_names MAT mod_en mod_resids mod_coeffs sum_CV_score max_pCs min_pCs pC CE interactions CV_score final_resids final_coeffs counts surf_normalized_counts coverages surf_energy sMax final_en INTERACTIONS mean_coeffs mf_resids
