...
% Must have CV_calc_v4.m in the active directory.
...
% v5: uses CV_calc_v4.m
% v6: outputs GS_ENERGY_BREAKDOWN.txt, the counts for the structures
% multiplied by the ECI values of the current CE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
mod_atat_names = atat_names(tokeep);
...
color_type{1} = 'black'; %Subsurface
%color_type{2} = 'blue'; %Surface
...
% CV score convergence tolerance
...
disp(CE)
[CV_score,std_dev,final_ECIs,ECI_errors] = CV_calc_v4(test_mat,energy,a,CV_tol);        %calulate the intial CV score
...
%%% v6 GS_ENERGY_BREAKDOWN.txt output

gs_energy_breakdown = zeros(numel(true_gs),numel(CE));
fID = fopen('GS_ENERGY_BREAKDOWN.txt','w');
fprintf(fID,'Ground State Structures:\n');
for ii = true_gs
    fprintf(fID,'%s\n',atat_names{ii});
end
fprintf(fID,'\n');
fclose(fID);
dlmwrite('GS_ENERGY_BREAKDOWN.txt',CE,'delimiter',' ','-append');

kk = 1;
for ii = true_gs
    if ads_flag == 1
        gs_counts = surf_normalized_counts(ii,CE)./coverages(ii);
        gs_counts(~isfinite(gs_counts)) = 0;
        gs_energy_breakdown(kk,:) = gs_counts.*final_coeffs';
    else
        gs_counts = surf_normalized_counts(ii,CE);
        gs_energy_breakdown(kk,:) = gs_counts.*final_coeffs';
    end
    kk = kk + 1;
end
dlmwrite('GS_ENERGY_BREAKDOWN.txt',gs_energy_breakdown,'delimiter',' ','-append');
...
for ii = CE
    ...
    if plot_clusters == 1 || plot_clusters == 2
        ...
        EIClab = ['EIC is ' num2str(final_coeffs(kk))];
        xlabel({cluslab , EIClab});
        hold on
        for jj = 1:nbodies
            type = the_types(jj);
            circles(X(jj),Y(jj),radius_type(type),'facecolor',color_type{type});
...
fprintf('----------------------------------------------------------------\n')
fprintf('----------------------------------------------------------------\n')
clearvars -except CE slope_counts slope_energy part_slope_en true_gs_slopes_counts true_gs_slopes part_ads_en ads_normalized_counts true_gs iso_gs diff_ads_counts diff_surf_counts diff_surf_energies diff_ads_energies true_gs_covs true_gs_ads_e  true_gs_ens true_gs_slopes ads_energy part_en orig_surf_normalized_counts atat_names newest_configs residuals mf_resids counts surf_normalized_counts coverages surf_energy sMax INTERACTIONS
