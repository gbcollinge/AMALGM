...
% (i.e. the 4th element)
    Site(:,1) = [0; 0 ; 0 ; 1]; % O site (4f hollow site)
    Site(:,2) = [1/2; 0 ; 0 ; 2]; % O site (bridge1 site)
    Site(:,3) = [0; 1/2 ; 0 ; 2]; % O site (bridge2 site)
    Site(:,4) = [-0.139359292000000; -0.139359292000000 ; 0 ; 3]; % O site (3f hollow site)
    ...
    for jj=1:num_configs                    % start working through the new configurations...   
       ...
       num_sites_here = size(SIG(SIG(:,3)==ii,:),1);
            for  ww= 1:num_sites_here
                clusters(ww,1) = non_linked_sites(ii);
                posclusters(ww,1) = non_linked_sites(ii);
                int_ptr = int_ptr + 1;
            end
        end
